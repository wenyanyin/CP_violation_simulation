...
fout=\$AGAMMAD0TOHHPI0DATADIR/data/DaVinciTuples_S24r1_part_Kpipi0.root
...
#../../../run python \$ANALYSISUTILSROOT/python/AnalysisUtils/addmva.py --inputfile ${inputfile/\$/\$} --inputtree "$inputtree" --outputfile $outputfile --outputtree $outputtree --weightsfile \$AGAMMAD0TOHHPI0ROOT/tmva/20180702-Lewis/TMVAClassification_BDT_Kpipi0.weights.xml --weightsvar ${mva} && \
../../../run \$ANALYSISUTILSROOT/scripts/mergetrees.py tmp.root ${inputfile/\$/\$} $inputtree ${outputfile/\$/\$} $outputtree && \
../../../run mv tmp.root ${fout/\$/\$}
