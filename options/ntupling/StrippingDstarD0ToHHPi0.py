...
def DstarMaker(_name,_KstDecays,_D0Decays,_DstDecays,_ChargedTracks,_Pi0s,_Slowpions,_Filter,config,prescale):
    ...
    KstSel = Selection("SelKst"+_name,
                       Algorithm = CombineParticles( name = "CombKst"+_name,
                                                     ...
                                                     CombinationCut = "(((ACHILD(PT,1) > %(Daughter_PT)s) & (ACHILD(BPVIPCHI2(),1) > %(Daughter_IPChi2)s)) | ((ACHILD(PT,2) > %(Daughter_PT)s) & (ACHILD(BPVIPCHI2(),2) > %(Daughter_IPChi2)s))) & (AM < 1850*MeV) & (ADOCACHI2CUT(%(HH_DOCAChi2)s,''))" % config,
                                                     MotherCut = "(VFASPF(VCHI2/VDOF) < %(HH_VtxChi2NDF)s) & (BPVVDCHI2 > %(HH_FDChi2)s)" % config),
    ...
    D0Comb = CombineParticles( name = "CombD0"+_name,
                               ...
                               MotherCut = "(VFASPF(VCHI2/VDOF) < %(D0_VtxChi2NDF)s) & (DMASS('D0') < %(D0_M_WINDOW)s *MeV) & (BPVDIRA > %(D0_DIRA)s)" %config)
    ...
    DstComb = CombineParticles( name = "CombDst"+_name,
                                ...
                                CombinationCut = "(AM - ACHILD(M,1) < %(DELTA_MASS_MAX)s+5 *MeV) & (ADOCACHI2CUT(%(Dst_DOCAChi2)s,''))" %config,
                                MotherCut = "(M - CHILD(M,1) < %(DELTA_MASS_MAX)s *MeV) & (VFASPF(VCHI2/VDOF) < %(Dst_VtxChi2NDF)s)" %config)
